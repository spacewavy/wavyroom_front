...
const OptionSelection = () => {
  ...
  return (
    <section className="sm:px-[24px] px-8 py-4">
          ...
          <div className="grid grid-cols-2 gap-4 pt-4">
            {options.map((o, index) => {
              return (
                <div
                  key={`options-${index}`}
                  className={`p-4 flex justify-between items-center border-[1px] rounded-xl border-[#E5E5E5]} `}
                >
                  <div className={`propName text-[12px] font-medium`}>
                    {o.option}
                  </div>
